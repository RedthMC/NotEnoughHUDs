package io.github.redth.notenoughhuds.utils;
...
public class Snapper extends DrawUtils {
    ...
    public void updateSnaps(int x, int y) {
        ...
        if (snappedX < 0 || snappedX + width >= mc.getWindow().getScaledWidth()) {
            lineX = -100;
        }
        if (snappedX == -100) {
            snappedX = x;
        ...
        if (snappedY < 0 || snappedY + height >= mc.getWindow().getScaledHeight()) {
            lineY = -100;
        }
        if (snappedY == -100) {
            snappedY = y;
    ...
    public void drawLines(MatrixStack matrix) {
        if (Screen.hasShiftDown()) return;

        if (lineX != -100) drawVerticalLine(matrix, lineX, 0.0F, screenHeight, LINE_COLOR);
        if (lineY != -100) drawHorizontalLine(matrix, 0.0F, screenWidth, lineY, LINE_COLOR);
    }
